...
public sealed class AdminConfigurationBackupService : IDisposable
{
    ...
    private static bool HasRuntimeFieldChanged(
        ...
        path switch
        {
            ...
            "MqttVision:JsonImport:MaxConcurrentImports" => current.JsonImport.MaxConcurrentImports != backup.JsonImport.MaxConcurrentImports,
            "MqttVision:JsonImport:MaxFileBytes" => current.JsonImport.MaxFileBytes != backup.JsonImport.MaxFileBytes,
            "MqttVision:JsonImport:AllowedExtensions" => !current.JsonImport.AllowedExtensions.Equals(backup.JsonImport.AllowedExtensions, StringComparison.OrdinalIgnoreCase),
